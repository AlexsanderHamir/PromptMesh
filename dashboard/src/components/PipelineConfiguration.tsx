...
interface PipelineConfigurationProps {
  ...
  onFormChange: (field: keyof PipelineForm, value: string) => void;
  errors: Record<string, string | null>;
  onFilesChange: (files: UploadedFile[]) => void;
}

export const PipelineConfiguration: React.FC<PipelineConfigurationProps> = ({
  pipelineForm,
  onFormChange,
  errors,
  onFilesChange,
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm">
                  ...
                  <div className="mt-2 text-xs text-blue-300">
                    <strong>Files:</strong>{" "}
                    {uploadedFiles.map((f: UploadedFile, i: number) => (
                      <span key={f.id}>
                        {f.metadata.name}
