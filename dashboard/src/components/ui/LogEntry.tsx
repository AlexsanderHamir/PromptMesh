...
interface LogEntryProps {
  log: LogEntryType;
  index: number;
  onAgentOutputClick?: (agentName: string, agentOutput: string, agentInput?: string) => void;
}

export const LogEntry: React.FC<LogEntryProps> = ({ log, index, onAgentOutputClick }) => {
  const logConfig = {
    [LOG_TYPES.INFO]: {
  ...
  const config = logConfig[log.level || LOG_TYPES.INFO] || logConfig[LOG_TYPES.INFO];
        ...
        return (
          <div className="mt-2 p-2 bg-green-500/10 rounded border border-green-500/20">
            <div className="flex items-center justify-between text-xs text-green-300">
              ...
              {onAgentOutputClick && agentOutput && (
                <button
                  onClick={() =>
                    onAgentOutputClick(
                      agent || "Unknown Agent",
